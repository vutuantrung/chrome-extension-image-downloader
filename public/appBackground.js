...
const MENU_SHOW_OVERLAY = "show-small-overlay";
const MENU_CHECK_INPUT = "menu-check-input";

const dateLength = "-------------".length;

const SAVE_ENDPOINT = "http://localhost:3125/api/saveIdentify";
const SEARCH_ENDPOINT = "http://localhost:3125/api/searchIdentify";
...
chrome.runtime.onInstalled.addListener(() => {
    ...
    chrome.contextMenus.create({
        ...
        title: "Check indentity",
        contexts: ["all"]
    });

    chrome.contextMenus.create({
        id: MENU_CHECK_INPUT,
        title: "Check input indentity...",
        contexts: ["all"]
    });
});
...
chrome.contextMenus.onClicked.addListener(async (info, tab) => {
    ...
    if (info.menuItemId === MENU_SEND_HIGHLIGHTED_TEXT) {
        ...
        saveSelectedText(selectedText).then((success) => {
            console.log("Sent highlighted text:", success);
        });
    }
    ...
    if (info.menuItemId === MENU_SHOW_OVERLAY) {
        try {
            const selectedText = (info.selectionText || "").trim();
            const withBase64Covers = await fetchSelectedText(selectedText);
            chrome.tabs.sendMessage(tab.id, {
                type: "MENU_SHOW_OVERLAY",
                data: withBase64Covers
            });
        } catch (err) {
    ...
    if (info.menuItemId === MENU_CHECK_INPUT) {
        try {
            chrome.tabs.sendMessage(tab.id, {
                type: "MENU_CHECK_INPUT",
                placeholder: "Type something and press Enter…"
            });
        } catch (err) {
            console.error("Failed to check input:", err);
        }
    }
});
...
function listeners(message, sender, sendResponse) {
    ...
    if (message.type === "PROCESS_FETCH_SELECTED_TEXT") {
        const selectedText = message.text || "";
        console.log('[selectedText]', selectedText);
        fetchSelectedText(selectedText).then((withBase64Covers) => {
            sendResponse({ data: withBase64Covers });
        });

        return true;
    }

    if (message.type === "PROCESS_SAVE_SELECTED_TEXT") {
        const selectedText = message.text || "";
        saveSelectedText(selectedText).then((success) => {
...
async function fetchSelectedText(selectedText) {
    try {
        if (!selectedText) return;

        const res = await fetch(SEARCH_ENDPOINT + "?identity=" + encodeURIComponent(selectedText), {
            method: "GET",
            headers: {
                "accept": "application/json",
                "content-type": "application/json"
        ...
        if (Array.isArray(resData)) {
            ...
            if (!resData?.code || !resData?.contentId) {
                console.error("No results found for:", selectedText);
                return;
            }
            dataFetched = [resData];
        }

        // console.log("[appBackground]", dataFetched);

        // Normalize + limit
        const normalized = dataFetched.slice(0, 8).map((it) => ({
            imageCover: typeof it?.thumbs === "string" ? it.thumbs : "",
            code: typeof it?.code === "string" ? it.code : "",
            createdTime: typeof it?.created_time === "number" ? new Date(it.created_time).toDateString() : 0
        }));

        // console.log('[normalized]', normalized);

        // Convert covers in background to base64 data URLs
        const withBase64Covers = await Promise.all(
            normalized.map(async (it) => ({
                ...it,
                imageCover: await toImageDataUrlSafe(it.imageCover)
            }))
        );
        return withBase64Covers;
    } catch (error) {
...
function suggestNewFilename(item, suggest) {
    ...
    let genre = (!task.options.genre || task.options.genre === "none") ? "" : `[${task.options.genre}]`;
...
async function toImageDataUrlSafe(url) {
    if (!url || typeof url !== "string") return "";
    if (url.startsWith("data:image/")) return url; // already base64

    try {
        // Some CDNs reject without headers; keep it simple first
        ...
        // Use server-provided mime if possible
