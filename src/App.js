...
import { isIncludedIn, removeSpecialCharacters, unique, parseHtmlString } from "./utils.js";
import {
    countImagesIndexes,
...
const App = () => {
    ...
    const filterMedias = () => {
        let medias =
            options.mode === "tab" ? tabsMedias : options.only_images_from_links === "true" ? linkedMedias : allMedias;
        // With images
        medias = options.with_images === "true" ? medias : medias.filter((media) => media.mediaType !== "image");
        // With videos
        medias = options.with_videos === "true" ? medias : medias.filter((media) => media.mediaType !== "video");
    ...
    useEffect(() => {
        ...
        if (options.fix_folder_name !== "true") setOptions((options) => ({ ...options, folder_name: "" }));
        if (options.fix_new_file_name !== "true") setOptions((options) => ({ ...options, new_file_name: "" }));
        if (options.fix_start_index !== "true") setOptions((options) => ({ ...options, start_index: 0 }));
        ...
        const collectMedias = async () => {
            ...
            if (pageName === "normal" || pageName === "jpgchurch" || pageName === "rule34") {
                allM = collectedMedias.allMediasLocal;
                linkedM = collectedMedias.linkedMediasLocal;
            }
            ...
            if (pageName === "telegram") {
                // Get title
                const titleDiv = currentPageInfor.dom.querySelector('header[class="tl_article_header"] > h1');
            ...
            setTabsMedias((tabsMedias) => unique([...tabsMedias, ...colectedTabMedias]));
    ...
    }, [options.fix_folder_name, options.fix_new_file_name, options.fix_start_index]);

    const mediasToDownload = useMemo(() => medias.filter(isIncludedIn(selectedMedias)), [medias, selectedMedias]);
